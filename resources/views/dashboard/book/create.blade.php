...
@section('main')
    ...
    <form action="{{ route('book.store') }}" method="POST" id="createBookForm" enctype="multipart/form-data">
        @csrf

        <div class="form-floating mb-3">
            <input type="text" name="title" class="form-control bg-dark text-light @error('title') is-invalid @enderror" id="title" placeholder=" " value="{{ old('title') }}">
        ...
        <div class="mb-3">
            <select name="category_id" class="w-100 @error('category_id') is-invalid @enderror" id="category">
                @php($oldCategory = old('category_id'))
        ...
        <div class="mb-3">
            Sampul
            <img class="img-fluid mt-1 mb-3 col-5 col-sm-3 col-md-2" id="bookCoverPreview">
        ...
        <div class="mb-3">
            <textarea name="synopsis" id="synopsis" cols="200" rows="5" class="form-control bg-dark text-light @error('synopsis') is-invalid @enderror" placeholder="Sinopsis">{{ old('synopsis') }}</textarea>
...
@push('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#createBookForm select').select2({
                theme: 'bootstrap-5',
                width: $(this).data('width') ? $(this).data('width') : $(this).hasClass('w-100') ? '100%' : 'style'
            })
        })

        $('#createBookForm input[name="cover"]').change(function() {
            const _image = document.querySelector('#createBookForm input[name="cover"]')
